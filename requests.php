                ...
                <div class="scroll-sidebar">
                    ...
                    <nav class="sidebar-nav active">
                        <ul id="sidebarnav" class="in">
                            ...
                            <li class="two-column">
                                <a class="has-arrow" href="./courses.php" aria-expanded="false"><i class="mdi mdi-school"></i><span class="hide-menu">Formación</span></a>
                                 <ul aria-expanded="false" class="collapse">
                    ...
                    <div class="col-12">
                        ...
                        <div class="card">
                            <div class="card-block">
                                <h4 class="card-title">Solicitudes de Cursos y Certificaciones</h4>
                                <h6 class="card-subtitle">Solicitudes de Cursos y Certificaciones</h6>
                                <div class="table-responsive m-t-40">
                                    <table id="requests" class="table table-bordered table-striped">
                                        <thead>
                                            <tr>
                                                <th>Solicitante</th>
                                                <th>Curso o Certificación</th>
                                        ...
                                        <tbody>
<?php
$sql = "SELECT courses.name AS cname, members.name AS mname, 
courses.id AS cid, members.id AS mid, courses_members.fecha_solicitud AS fsolicitado, 
courses_members.status_solicitud AS ssolicitado, courses_members.comentarios_solicitud AS csolicitado
        FROM courses, members, courses_members 
		WHERE courses_members.user_id=members.id
		AND courses_members.course_id=courses.id
		AND	courses_members.solicitado='si' 
		AND courses_members.active='Inactivo'
        ...
        {           echo '<tr> 
                    <td>'.$row['mname'].'</td>
                    <td>'.$row['cname'].'</td>
                    <td>'.$row['fsolicitado'].'</td>
					<td>'.$row['ssolicitado'].'</td>
					<td><a href="editrequest.php?cid='.$row['cid'].'&cname='.urlencode($row['cname']).'&mid='.$row['mid'].'&mname='.urlencode($row['mname']).'&status='.$row['ssolicitado'].'&comments='.urlencode($row['csolicitado']).'" class="btn btn-danger">Editar</a></td>
                </tr>';
        }?> 
                                        </tbody>
                                    </table>
