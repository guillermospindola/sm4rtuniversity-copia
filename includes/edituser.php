                        ...
                        <div class="card" style="height:457px;">
                            <div class="card-block" >
                                ...
                                <input hidden id="userid" name="userid" value="<?php echo $_GET['id']; ?>">
                                                    ...
                                                    <input required type="text" id="area" name="area" class="form-control" value="<?php echo $_GET['area']; ?>" >
                                                      <small class="form-control-feedback">Área a donde pertenece el usuario</small>  </div>

                                                      <div class="form-group">
                                                    <label class="control-label">Dirección</label>
                                                    <input required type="text" id="direccion" name="direccion" class="form-control" value="<?php echo $_GET['direccion']; ?>" >
                                                      <small class="form-control-feedback">Dirección a donde pertenece el usuario</small>  </div>

                                        

                                                      <div class="form-actions" style="position: absolute; top: 400px;">
...
$sqlc = "SELECT members.id AS mid, courses_members.course_id as cmcid, courses_members.suma as suma, courses_members.renovacion AS renovacion,
...
ORDER BY cname";
		...
		$queryc = mysqli_query($mysqli, $sqlc); 
		 $cert = array();
		...
		if($row['cmact']=='Activo')
			...
			else
			{
				$checkcert="";
					...
					echo ' <input id="'.$row['cid'].'" name="'.$row['cid'].'" value="'.$row['cid'].'" type="checkbox" '.$checkcert.'> <label for="'.$row['cid'].'"><a href="certvalidity.php?courseid='.$row['cid'].'&coursename='.$row['cname'].'&userid='.$userid.'&renovacion='.$row['renovacion'].'&suma='.$row['suma'].'">'.$row['cname'].'</a></label><br>';
					array_push($cert, $row['cid']);
		}
		
		$sqlall="SELECT id, name FROM courses WHERE active='Activo' AND type='Certificacion' ORDER BY name";
...
$sql = "SELECT members.id AS mid, courses.type AS ctype, courses_members.type AS cmtype, courses_members.course_id as cmcid, courses_members.user_id AS cmuid, courses_members.active AS cmact, courses.name AS cname, courses.id AS cid, courses.active AS cact, courses.type, courses_members.suma as suma, courses_members.renovacion AS renovacion
FROM members, courses_members, courses
WHERE members.id=courses_members.user_id
AND courses.id = courses_members.course_
AND courses.type = 'Curso'
AND members.id ='".$userid."' ORDER BY cname";

$sqlall="SELECT id, name FROM courses WHERE active='Activo' AND type='Curso' ORDER BY name";
		...
		{			
			...
			else
			{
				$checked="";
					...
					echo ' <input id="'.$row['cid'].'" name="'.$row['cid'].'" value="'.$row['cid'].'" type="checkbox" '.$checked.'> <label for="'.$row['cid'].'"><a href="certvalidity.php?courseid='.$row['cid'].'&coursename='.$row['cname'].'&renovacion='.$row['renovacion'].'&userid='.$userid.'&suma='.$row['suma'].'">'.$row['cname'].'</a></label><br>';
