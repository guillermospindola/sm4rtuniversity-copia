...
$url = "univeristy.sm4rt.com/uploads/";
?>
...
$sql = "SELECT * FROM proposals";
	...
	if (strlen($row['description']) > 50)
			...
			{$row['ganador'] = 'no';}
                    ...
                    <td>'.$row['leader'].'</td>
                    <td>'.$row['value'].'</td>
					<td><a href="./uploads/'.htmlentities($attachment[2]).'">Documento</a></td>
                    <td><a href="./editproject.php?id='.$row['id'].'&name='.urlencode($row['name']).'&description='.urlencode($row['description']).'&leader='.$row['leader'].'&member1='.$row['member1'].'&member2='.$row['member2'].'&status='.$row['status'].'&value='.$row['value'].'&ganador='.$row['ganador'].'" class="btn btn-danger">Editar</a></td></tr>';
        }?> 
                                        </tbody>
                                    </table>
                                </div>
